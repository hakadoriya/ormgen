...
const SelectUserForUpdateByPKQuery = `SELECT user_id, username, address, group_id FROM user WHERE user_id = ? FOR UPDATE`
...
const SelectUserForUpdateByUsernameQuery = `SELECT user_id, username, address, group_id FROM user WHERE username = ? FOR UPDATE`
...
const SelectUserForUpdateByUsernameAndAddressQuery = `SELECT user_id, username, address, group_id FROM user WHERE (username = ? AND address = ?) FOR UPDATE`
...
const SelectAdminUserForUpdateByPKQuery = `SELECT admin_user_id, username, group_id FROM admin_user WHERE admin_user_id = ? FOR UPDATE`
