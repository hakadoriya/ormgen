...
func Generate(c *cliz.Command, args []string) (err error) {
	...
	logs.Otel.InfoContext(ctx, fmt.Sprintf("🔆 start %s", strings.Join(c.GetExecutedCommandNames(), " ")), slog.Any("os.Args", os.Args))
	defer logs.Otel.InfoContext(ctx, fmt.Sprintf("💤 end %s", strings.Join(c.GetExecutedCommandNames(), " ")), slog.Any("os.Args", os.Args))
