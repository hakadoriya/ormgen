...
lint:  ## Run secretlint, go mod tidy, golangci-lint
	...
	# tidy
	time go-mod-tidy-all
	# golangci-lint
	# ref. https://golangci-lint.run/usage/linters/
	time golangci-lint run --fix --print-resources-usage --sort-results --verbose --timeout=5m
...
up:  ## Run docker compose up --wait -d
	...
	@printf '[\033[36mNOTICE\033[0m] %s\n' "Check Jaeger UI: http://localhost:16686/search?limit=20&lookback=1h&maxDuration&minDuration&service=ormgen"
	@if [ `uname -s` = Darwin ]; then \
		open 'http://localhost:16686/search?limit=20&lookback=1h&maxDuration&minDuration&service=ormgen' ; \
